...
%   1,1.  For now, the only world available is 'fig72' from Fig 7.2 of
...
%   have one world fig72, restart just reinitializes this world.
...
% initialize_world(World): Initializes the Wumpus world in Figure 7.2 of
%                          [Russell & Norvig]
%
...
initialize_world(fig72) :-
  ...
  retractall(gold(_,_)),
  retractall(pit(_,_)),
  assertz(wumpus_world_extent(4)),
  assertz(wumpus_location(1,3)),
  assertz(wumpus_health(alive)),
  assertz(gold(2,3)),
  assertz(pit(3,1)),
  assertz(pit(3,3)),
  assertz(pit(4,4)).
...
initialize_agent :-
  ...
  assertz(agent_orientation(0)),
  assertz(agent_in_cave(yes)),
  assertz(agent_health(alive)),
  assertz(agent_gold(0)),
  assertz(agent_arrows(1)),
  assertz(agent_score(0)).
...
execute(turnleft,[Stench,Breeze,Glitter,no,no]) :-
  ...
  assertz(agent_orientation(NewAngle)),
  stench(Stench),
  breeze(Breeze),
  glitter(Glitter).
...
execute(climb,[no,no,no,no,no]) :-
  ...
  S1 is (S + (1000 * G)),
  assertz(agent_score(S1)),
  retract(agent_in_cave(yes)),
  assertz(agent_in_cave(no)).
